...
# -*- coding: utf-8 -*-
"""
run_v4_1_download_and_audit.py

v4.1 배치 결과 다운로드 + 오딧 일괄 실행 스크립트.

Usage:
    python3 run_v4_1_download_and_audit.py

    # 배치 상태만 확인 (다운로드 없음):
    python3 run_v4_1_download_and_audit.py --check-only

    # 이미 다운로드된 output 파일로 오딧만 실행:
    python3 run_v4_1_download_and_audit.py --audit-only
"""
...
BATCH_ID = "batch_6a3259795c208190b48bc16de6296eb6"

REQUESTS_JSONL = Path(
    "/Users/hgs/Desktop/IISE CD/data/interim/"
    "pr06a_stock_news_sample_requests_from_briefs_v4_1_dart_detail_context/"
...
OUTPUT_JSONL = OUTPUT_DIR / "stock_news_sample_outputs_v4_1.jsonl"
ERROR_JSONL  = OUTPUT_DIR / "stock_news_sample_errors_v4_1.jsonl"
AUDIT_DIR    = OUTPUT_DIR / "audit_v4_1"
...
def _write_audit_csv(rows: list[AuditRow], audit_dir: Path) -> None:
    path = audit_dir / "generated_news_lines_audit_v4_1.csv"
    fieldnames = [
...
def _write_audit_report(
    ...
    path = audit_dir / "generated_news_lines_audit_report_v4_1.md"
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"[saved] {path}")
...
def _print_audit_summary(rows: list[AuditRow]) -> None:
    ...
    passed = sum(1 for r in rows if r.pass_all)
    print("=" * 100)
    print(f"[v4.1 audit]  total={total}  pass={passed}  fail={total - passed}  "
          f"rate={passed/total:.1%}" if total else "[v4.1 audit] no rows")
    print("=" * 100)
    for row in rows:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="v4.1 배치 다운로드 + 오딧")
