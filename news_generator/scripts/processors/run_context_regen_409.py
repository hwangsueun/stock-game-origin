#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""v8 맥락 레이어(가격·섹터) 정정분 재생성 배치 러너.

inject_context_layers로 market_context가 주입되어 writer payload가 바뀐 409건만
컨텍스트 인식 프롬프트로 다시 생성한다. 30건 청크 순차 제출(OpenAI Batch
enqueued token 한도 90k). run_v4_2_regen_unit_fix.py와 동일 로직, 경로만 다름.

요청 출처: data/interim/context_layers/regeneration_409/requests.jsonl
  (단위버그 정정본 위에서 빌드됨 — 매출 수치는 정정값을 그대로 보존)

사용법:
  cd "/Users/hgs/Desktop/IISE-CD/data-pipeline/news_generator"
  python scripts/processors/run_context_regen_409.py
"""
...
REGEN_DIR = BASE_DIR / "data/interim/context_layers/regeneration_409"
REGEN_REQUESTS = REGEN_DIR / "requests.jsonl"
...
METADATA_PREFIX = "context_regen_409"
...
def main() -> None:
    ...
    total = len(all_lines)
    print(f"[init] 맥락 재생성 요청: {total}건")
    ...
    for start in range(0, total, CHUNK_SIZE):
        ...
        # 재개 시 경로 필드는 절대경로(BASE_DIR 기준)를 유지하고,
        # 배치 추적 필드만 로드한다. (기존 state.json에 저장된 상대경로가
        # cwd와 어긋나 FileNotFoundError가 나는 것을 방지)
        path_fields = {"input_jsonl", "output_jsonl", "error_jsonl", "state_json"}
