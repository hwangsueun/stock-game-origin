...
class EventFamilyClassifier:
    ...
    INVESTMENT_TERMS = ["시설투자", "신규투자", "투자결정", "증설", "공장", "라인", "capex"]
    TREASURY_TERMS = ["자기주식", "자사주", "신탁계약", "자기주식취득", "자기주식처분"]
    ASSET_TRANSACTION_TERMS = ["유형자산취득", "유형자산처분", "유형자산 취득", "유형자산 처분"]
    EQUITY_INVESTMENT_TERMS = [
        "타법인주식", "출자증권", "주식및출자증권", "주식 및 출자증권",
        "타법인 주식", "출자 증권", "주식양수", "주식양도",
    ]
    CAPITAL_FINANCING_TERMS = [
        "유상증자", "무상증자", "전환사채", "교환사채", "신주인수권",
        "사채권발행", "사채권 발행", "감자결정", "감자 결정", "증자결정",
    ]
    BUSINESS_TRANSFER_TERMS = [
        "영업양수", "영업양도", "영업 양수", "영업 양도", "회사합병",
        "합병결정", "분할결정", "분할합병", "포괄적교환", "포괄적 교환",
        "주식의포괄적교환", "주식의 포괄적 교환", "이전결정",
    ]
    TRADING_STATUS_TERMS = ["매매거래정지", "거래정지", "정지해제", "중요내용공시"]
    LISTING_RISK_TERMS = ["투자유의", "관리종목", "상장폐지", "상장 폐지", "지정 우려", "상장폐지 우려"]
    LEGAL_TERMS = [
        "소송", "판결", "제재", "과징금", "조사", "검찰", "공정위", "금감원",
        "분쟁", "손해배상", "횡령", "배임", "혐의발생", "사실확인",
    ]
    MANAGEMENT_TERMS = ["대표이사", "임원", "최대주주", "지분", "경영권", "이사회", "주주총회"]
    MAJOR_MGMT_TERMS = ["투자판단관련주요경영사항", "투자판단 관련 주요경영사항"]
    ...
    def classify_text(cls, text: str) -> str:
        ...
        if cls._contains_any(s, cls.CONTRACT_TERMS):
            return "contract"
        if cls._contains_any(s, cls.TREASURY_TERMS):
            return "treasury_stock"
        if cls._contains_any(s, cls.ASSET_TRANSACTION_TERMS):
            return "asset_transaction"
        if cls._contains_any(s, cls.EQUITY_INVESTMENT_TERMS):
            return "equity_investment"
        if cls._contains_any(s, cls.CAPITAL_FINANCING_TERMS):
            return "capital_financing"
        if cls._contains_any(s, cls.BUSINESS_TRANSFER_TERMS):
            return "business_transfer"
        if cls._contains_any(s, cls.TRADING_STATUS_TERMS):
            return "trading_status"
        if cls._contains_any(s, cls.LISTING_RISK_TERMS):
            return "listing_risk"
        if cls._contains_any(s, cls.LEGAL_TERMS):
            return "legal_regulatory"
        if cls._contains_any(s, cls.MAJOR_MGMT_TERMS):
            return "major_management_matter"
...
class FamilyCompatibility:
    COMPATIBLE = {
        ...
        ("management_governance", "major_management_matter"),
        ("major_management_matter", "management_governance"),
        ("other_company_event", "major_management_matter"),
        ("major_management_matter", "other_company_event"),
    }
...
class EventGroupConfig:
    ...
    # DART can easily explode the group count if every disclosure row becomes an anchor.
    # Default scope is intentionally limited to stocks present in stock_event annotations.
    # Use --dart-scope all only when you intentionally want DART-only groups for the full universe.
    dart_scope: str = "stock_event_universe"  # all | stock_event_universe | stock_event_anchor_universe
    dart_require_is_event: bool = True
    min_dart_materiality_score: float = 1.0
    dart_include_families: Tuple[str, ...] = (
        "earnings",
        "guidance",
        "dividend",
        "contract",
    ...
    exclude_periodic_dart_report_anchors: bool = True
...
class DartLoader:
    ...
    def to_anchor_items(
        df: pd.DataFrame,
        target_stock_codes: Optional[set[str]] = None,
        require_is_dart_event: bool = True,
        min_materiality_score: float = 1.0,
        include_families: Optional[set[str]] = None,
        exclude_periodic_report_anchors: bool = True,
    ) -> List[EvidenceItem]:
        """Convert filtered DART rows into official topic anchors.

        Important: DART is official topic evidence, not automatic market-cause evidence.
        This method is intentionally conservative because raw DART files can contain many
        routine or weak disclosures. If every DART row becomes an anchor, the output becomes
        thousands of low-value groups and defeats the purpose of event grouping.
        """
        if df.empty:
            return []
        ...
        if target_stock_codes is not None:
            target_stock_codes = {normalize_stock_code(x) for x in target_stock_codes if normalize_stock_code(x)}
            use = use[use["stock_code"].map(normalize_stock_code).isin(target_stock_codes)].copy()

        if require_is_dart_event and "is_dart_event" in use.columns:
            use = use[use["is_dart_event"].map(parse_bool)].copy()

        if min_materiality_score is not None and "dart_materiality_score" in use.columns:
            scores = use["dart_materiality_score"].map(lambda x: safe_float(x) if safe_float(x) is not None else 0.0)
            use = use[scores >= float(min_materiality_score)].copy()

        if include_families:
            use = use[use["event_family"].isin(include_families)].copy()

        if exclude_periodic_report_anchors:
            use = use[~use.apply(DartLoader._is_plain_periodic_report_anchor, axis=1)].copy()
    ...
    @staticmethod
    def _is_plain_periodic_report_anchor(row: pd.Series) -> bool:
        """Exclude plain annual/quarterly/half-year report rows as DART anchors.

        These reports are official documents, but as event anchors they are usually too broad.
        Specific disclosures such as provisional earnings, dividend decision, contract,
        investment decision, lawsuit, etc. should remain anchors.
        """
        report = TextNormalizer.normalize(
            " ".join([
                clean_str(row.get("report_name_clean")),
                clean_str(row.get("report_name")),
            ])
        ...
        if not any(re.search(pat, report) for pat in periodic_patterns):
            return False

        # Keep a periodic-looking row only if it also contains a specific event keyword.
        specific_terms = (
            EventFamilyClassifier.DIVIDEND_TERMS
            + EventFamilyClassifier.GUIDANCE_TERMS
            + EventFamilyClassifier.CONTRACT_TERMS
            + EventFamilyClassifier.INVESTMENT_TERMS
            + EventFamilyClassifier.LEGAL_TERMS
            + EventFamilyClassifier.MANAGEMENT_TERMS
        )
        if EventFamilyClassifier._contains_any(report, specific_terms):
            return False
        return True
            ...
            rows.append(
                {
                    ...
                    "has_dart": pre.get("has_dart", pre.get("has_official_evidence")),
                    "has_stock_event_trigger": pre.get("has_stock_event_trigger"),
                    "has_stock_event_context": pre.get("has_stock_event_context"),
...
class EventGroupPipeline:
    ...
    def run(self) -> None:
        ...
        stock_df = self._filter_date_range(StockEventLoader(self.config.stock_event_path).load(), "stock_event")
        dart_df = self._filter_date_range(DartLoader(self.config.dart_path).load(), "dart")
        ...
        dart_target_codes = self._resolve_dart_target_codes(stock_df, stock_anchor_items)
        dart_items = DartLoader.to_anchor_items(
            dart_df,
            target_stock_codes=dart_target_codes,
            require_is_dart_event=self.config.dart_require_is_event,
            min_materiality_score=self.config.min_dart_materiality_score,
            include_families=set(self.config.dart_include_families) if self.config.dart_include_families else None,
            exclude_periodic_report_anchors=self.config.exclude_periodic_dart_report_anchors,
        )
        ...
        print(f"[config] start_date: {self.config.start_date}")
        print(f"[config] end_date: {self.config.end_date}")
        print(f"[config] dart_scope: {self.config.dart_scope}")
        print(f"[config] dart_require_is_event: {self.config.dart_require_is_event}")
        print(f"[config] min_dart_materiality_score: {self.config.min_dart_materiality_score}")
        print(f"[config] exclude_periodic_dart_report_anchors: {self.config.exclude_periodic_dart_report_anchors}")
        print(f"[config] dart_target_stock_count: {len(dart_target_codes) if dart_target_codes is not None else 'ALL'}")
    ...
    def _filter_date_range(self, df: pd.DataFrame, label: str) -> pd.DataFrame:
        """Apply configured inclusive date range to a loaded table.

        This method is intentionally kept in the pipeline layer because some
        loaders are schema-tolerant and should remain focused on normalization.
        """
        if df.empty:
            return df
        ...
        out = df.copy()
        out["event_date"] = to_date_series(out["event_date"])
        out = out[out["event_date"].notna()].copy()

        start = to_date_value(self.config.start_date)
        end = to_date_value(self.config.end_date)

        if start is not None:
            out = out[out["event_date"] >= start]
        if end is not None:
            out = out[out["event_date"] <= end]

        return out.copy()


    def _resolve_dart_target_codes(
        self,
        stock_df: pd.DataFrame,
        stock_anchor_items: List[EvidenceItem],
    ) -> Optional[set[str]]:
        scope = clean_str(self.config.dart_scope).lower()
        if scope == "all":
            return None
        if scope == "stock_event_anchor_universe":
            return {x.stock_code for x in stock_anchor_items if x.stock_code}
        if scope == "stock_event_universe":
            if stock_df.empty or "stock_code" not in stock_df.columns:
                return set()
            return {normalize_stock_code(x) for x in stock_df["stock_code"].dropna().tolist() if normalize_stock_code(x)}
        raise ValueError(
            "dart_scope must be one of: all, stock_event_universe, stock_event_anchor_universe"
        )
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--start-date", type=str, default="2013-01-01")
    ap.add_argument("--end-date", type=str, default="2023-12-31")
    ...
    ap.add_argument(
        "--dart-scope",
        type=str,
        default="stock_event_universe",
        choices=["all", "stock_event_universe", "stock_event_anchor_universe"],
        help="DART anchor scope. Default prevents all-DART explosion by limiting DART to stocks in stock_event annotations.",
    )
    ap.add_argument(
        "--dart-require-is-event",
        action="store_true",
        default=True,
        help="Require is_dart_event=True when the column exists. Enabled by default.",
    )
    ap.add_argument(
        "--allow-non-event-dart",
        action="store_true",
        help="Disable the is_dart_event=True requirement.",
    )
    ap.add_argument("--min-dart-materiality-score", type=float, default=1.0)
    ap.add_argument(
        "--dart-include-families",
        type=str,
        default="earnings,guidance,dividend,contract,investment,treasury_stock,asset_transaction,equity_investment,capital_financing,business_transfer,trading_status,listing_risk,legal_regulatory,management_governance,major_management_matter,product_supply_chain,other_company_event",
        help="Comma-separated event families allowed as DART anchors. Empty string means no family filter.",
    )
    ap.add_argument(
        "--include-periodic-dart-report-anchors",
        action="store_true",
        help="Allow plain 사업보고서/분기보고서/반기보고서 rows to become event anchors.",
    )
...
def main() -> None:
    ...
    config = EventGroupConfig(
        ...
        dart_require_is_event=(not args.allow_non_event_dart),
        min_dart_materiality_score=args.min_dart_materiality_score,
        dart_include_families=tuple(
            x.strip() for x in str(args.dart_include_families).split(",") if x.strip()
        ),
        exclude_periodic_dart_report_anchors=(not args.include_periodic_dart_report_anchors),
    )
